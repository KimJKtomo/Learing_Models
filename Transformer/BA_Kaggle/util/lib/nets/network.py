...
class Network(object):
    ...
    def _add_act_summary(self, tensor):
        ...
        tf.summary.scalar(
            "ACT/" + tensor.op.name + "/zero_fraction", tf.nn.zero_fraction(tensor)
        )
    ...
    def _reshape_layer(self, bottom, num_dim, name):
        ...
        with tf.variable_scope(name) as scope:
            ...
            reshaped = tf.reshape(
                to_caffe, tf.concat(axis=0, values=[[1, num_dim, -1], [input_shape[2]]])
            )
    ...
    def _crop_pool_layer(self, bottom, rois, name):
        with tf.variable_scope(name) as scope:
            ...
            height = (tf.to_float(bottom_shape[1]) - 1.0) * np.float32(
                self._feat_stride[0]
            )
            width = (tf.to_float(bottom_shape[2]) - 1.0) * np.float32(
                self._feat_stride[0]
            )
    ...
    def _smooth_l1_loss(
        ...
        smoothL1_sign = tf.stop_gradient(
            tf.to_float(tf.less(abs_in_box_diff, 1.0 / sigma_2))
        )
    ...
    def _region_proposal(self, net_conv, is_training, initializer):
        ...
        rpn_cls_pred = tf.argmax(
            tf.reshape(rpn_cls_score_reshape, [-1, 2]), axis=1, name="rpn_cls_pred"
        )
