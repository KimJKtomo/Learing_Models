...
class DABDeformableDETR(nn.Module):
    ...
    def __init__(self, backbone, transformer, num_classes, num_queries, num_feature_levels,
        ...
        self.label_enc = nn.Embedding(
            num_classes + 1, hidden_dim - 1)  # # for indicator
        if not two_stage:
            if not use_dab:
                self.query_embed = nn.Embedding(num_queries, hidden_dim*2)
            else:
                self.tgt_embed = nn.Embedding(
                    num_queries, hidden_dim-1)  # for indicator
...
class SetCriterion(nn.Module):
    ...
    def forward(self, outputs, targets, mask_dict=None):
        ...
        dn_losses = compute_dn_loss(
            mask_dict, self.training, aux_num, self.focal_alpha)
...
def build_dab_deformable_detr(backbone_name):
    ...
    # dn loss
    weight_dict['tgt_loss_ce'] = 1
    weight_dict['tgt_loss_bbox'] = 5
    weight_dict['tgt_loss_giou'] = 2

    losses = ['labels', 'boxes', 'cardinality']

    # num_classes, matcher, weight_dict, losses, focal_alpha=0.25
    criterion = SetCriterion(num_classes, matcher,
                             weight_dict, losses, focal_alpha=0.25)
