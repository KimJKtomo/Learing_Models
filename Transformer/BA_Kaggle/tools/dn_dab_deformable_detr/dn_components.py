...
def prepare_for_dn(dn_args, tgt_weight, embedweight, batch_size, training, num_queries, num_classes, hidden_dim, label_enc):
    ...
    if training:
        ...
        input_query_label = torch.cat(
            [padding_label, tgt], dim=0).repeat(batch_size, 1, 1)
        input_query_bbox = torch.cat(
            [padding_bbox, refpoint_emb], dim=0).repeat(batch_size, 1, 1)
...
def compute_dn_loss(mask_dict, training, aux_num, focal_alpha):
    ...
    if training and 'output_known_lbs_bboxes' in mask_dict:
        ...
        losses.update(tgt_loss_labels(
            output_known_class[-1], known_labels, num_tgt, focal_alpha))
        losses.update(tgt_loss_boxes(
            output_known_coord[-1], known_bboxs, num_tgt))
    else:
    ...
    if aux_num:
        for i in range(aux_num):
            ...
            if training and 'output_known_lbs_bboxes' in mask_dict:
                ...
                l_dict = tgt_loss_boxes(
                    output_known_coord[i], known_bboxs, num_tgt)
                l_dict = {k + f'_{i}': v for k, v in l_dict.items()}
                losses.update(l_dict)
            else:
                l_dict = dict()
